...
class FilckrPhotpAPI {
    // get photo
    class func getPhoto(model: Photos, searchName: String, page: Int, perPage: Int, competion: @escaping(_ model: Photos) -> Void){
        //API
        ...
        HttpTask.dataTask(url: url, body: body, header: header, httpMethod: .Get) { (resultArray) in
            ...
            competion(ConvertResult.photoToModel(model: model, result: result))
        }
    }
}
